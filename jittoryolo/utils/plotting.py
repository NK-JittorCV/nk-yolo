...
from jittoryolo.utils import IS_COLAB, IS_KAGGLE, LOGGER, TryExcept, ops, plt_settings, threaded
from jittoryolo.utils.checks import check_font, check_version, is_ascii
from jittoryolo.utils.files import increment_path


class Colors:
    """
    jittoryolo color palette https://docs.jittoryolo.com/reference/utils/plotting/#jittoryolo.utils.plotting.Colors.

    This class provides methods to work with the jittoryolo color palette, including converting hex color codes to
    RGB values.
    ...
    !!! note "jittoryolo Brand Colors"

        For jittoryolo brand colors see [https://www.jittoryolo.com/brand](https://www.jittoryolo.com/brand). Please use the official jittoryolo colors for all marketing materials.
    """
...
class Annotator:
    """
    jittoryolo Annotator for train/val mosaics and JPGs and predictions annotations.
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        colors = jt.Var(colors, device=masks.device, dtype=jt.float32) / 255.0  # shape(n,3)
    ...
    def kpts(self, kpts, shape=(640, 640), radius=None, kpt_line=True, conf_thres=0.25, kpt_color=None):
        ...
        Args:
            kpts (jt.Var): Keypoints, shape [17, 3] (x, y, confidence).
...
@TryExcept()  # known issue https://github.com/jittoryolo/yolov5/issues/5395
@plt_settings()
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    """Plot training labels including class histograms and box statistics."""
    import pandas  # scope for faster 'import jittoryolo'
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    Args:
        xyxy (jt.Var or list): A tensor or list representing the bounding box in xyxy format.
    ...
    Example:
        ```python
        from jittoryolo.utils.plotting import save_one_box
        ...
        ```
    """
    if not isinstance(xyxy, jt.Var):  # may be list
        xyxy = jt.stack(xyxy)
    ...
    if save:
        ...
        # cv2.imwrite(f, crop)  # save BGR, https://github.com/jittoryolo/yolov5/issues/7007 chroma subsampling issue
...
@threaded
def plot_images(
    images: Union[jt.Var, np.ndarray],
    batch_idx: Union[jt.Var, np.ndarray],
    cls: Union[jt.Var, np.ndarray],
    bboxes: Union[jt.Var, np.ndarray] = np.zeros(0, dtype=np.float32),
    confs: Optional[Union[jt.Var, np.ndarray]] = None,
    masks: Union[jt.Var, np.ndarray] = np.zeros(0, dtype=np.uint8),
    kpts: Union[jt.Var, np.ndarray] = np.zeros((0, 51), dtype=np.float32),
    ...
    if isinstance(cls, jt.Var):
        cls = cls.cpu().numpy()
    if isinstance(bboxes, jt.Var):
        bboxes = bboxes.cpu().numpy()
    if isinstance(masks, jt.Var):
        masks = masks.cpu().numpy().astype(int)
    if isinstance(kpts, jt.Var):
        kpts = kpts.cpu().numpy()
    if isinstance(batch_idx, jt.Var):
...
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, on_plot=None):
    ...
    Example:
        ```python
        from jittoryolo.utils.plotting import plot_results

        plot_results("path/to/results.csv", segment=True)
        ```
    """
    import pandas as pd  # scope for faster 'import jittoryolo'
...
def output_to_target(output, max_det=300):
    ...
    for i, o in enumerate(output):
        ...
        j = jt.full((conf.shape[0], 1), i)
        targets.append(jt.cat((j, cls, ops.xyxy2xywh(box), conf), 1))
    targets = jt.cat(targets, 0).numpy()
...
def output_to_rotated_target(output, max_det=300):
    ...
    for i, o in enumerate(output):
        ...
        j = jt.full((conf.shape[0], 1), i)
        targets.append(jt.cat((j, cls, box, angle, conf), 1))
    targets = jt.cat(targets, 0).numpy()
    return targets[:, 0], targets[:, 1], targets[:, 2:-1], targets[:, -1]
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    Args:
        x (jt.Var): Features to be visualized.
    ...
    if isinstance(x, jt.Var):
        ...
        if height > 1 and width > 1:
            ...
            blocks = jt.chunk(x[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
