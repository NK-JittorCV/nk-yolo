...
from jittoryolo.utils import DEFAULT_CFG, LOGGER, colorstr
from jittoryolo.utils.jittor_utils import autocast, profile
...
def autobatch(model, imgsz=640, fraction=0.60, batch_size=DEFAULT_CFG.batch):
    ...
    properties = jt.cuda.get_device_properties(device)  # device properties
    t = properties.total_memory / gb  # GiB total
    r = jt.cuda.memory_reserved(device) / gb  # GiB reserved
    a = jt.cuda.memory_allocated(device) / gb  # GiB allocated
    f = t - (r + a)  # GiB free
    ...
    try:
        img = [jt.empty(b, 3, imgsz, imgsz) for b in batch_sizes]
