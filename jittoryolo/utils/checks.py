...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='jittoryolo'.
    ...
    Example:
        ```python
        from jittoryolo.utils.checks import parse_requirements

        parse_requirements(package="jittoryolo")
        ```
    """
    if package:
...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    stride = int(stride.max() if isinstance(stride, jt.Var) else stride)
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='jittoryolo'
        try:
...
def check_latest_pypi_version(package_name="jittoryolo"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the jittoryolo package is available on PyPI.
    ...
    if ONLINE and IS_PIP_PACKAGE:
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    f"New https://pypi.org/project/jittoryolo/{latest} available 😃 "
                    f"Update with 'pip install -U jittoryolo'"
                )
                return True
        except Exception:
            pass
...
def check_font(font="Arial.ttf"):
    ...
    url = f"https://github.com/jittoryolo/assets/releases/download/v0.0.0/{name}"
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Example:
        ...
        check_requirements("jittoryolo>=8.0.0")

        # Check multiple packages
        check_requirements(["numpy", "jittoryolo>=8.0.0"])
        ```
    """
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} jittoryolo requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
        ...
        if "u.yaml" in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    ...
                    f"trained with https://github.com/jittoryolo/jittoryolo and feature improved performance vs "
                    f"standard YOLOv5 models trained with https://github.com/jittoryolo/yolov5.\n"
                )
    return file
...
def check_yolo(verbose=True, device=""):
    ...
    from jittoryolo.utils.jittor_utils import select_device
...
def collect_system_info():
    ...
    from jittoryolo.utils import ENVIRONMENT  # scope to avoid circular import
    from jittoryolo.utils.jittor_utils import get_cpu_info, get_gpu_info

    gib = 1 << 30  # bytes per GiB
    cuda = jt and jt.cuda.is_available()
    check_yolo()
    total, used, free = shutil.disk_usage("/")
    ...
    info_dict = {
        ...
        "GPU count": jt.cuda.device_count() if cuda else None,
        "CUDA": jt.version.cuda if cuda else None,
    }
    LOGGER.info("\n" + "\n".join(f"{k:<20}{v}" for k, v in info_dict.items()) + "\n")

    package_info = {}
    for r in parse_requirements(package="jittoryolo"):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from jittoryolo.utils.checks import check_amp
    ...
    """
    from jittoryolo.utils.jittor_utils import autocast
    ...
    try:
        ...
        LOGGER.warning(
            ...
            f"Unable to load YOLO11n for AMP checks due to possible jittoryolo package modifications. {warning_msg}"
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer jittoryolo HUB URLs by stripping potential authentication information."""
