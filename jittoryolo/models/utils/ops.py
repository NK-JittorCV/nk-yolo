...
from jittoryolo.utils.metrics import bbox_iou
from jittoryolo.utils.ops import xywh2xyxy, xyxy2xywh
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        pred_scores = jt.sigmoid(pred_scores) if self.use_fl else jt.softmax(pred_scores, dim=-1)
...
def get_cdn_group(
    ...
    map_indices = jt.concat([map_indices + max_nums * i for i in range(2 * num_group)])
    padding_cls[(dn_b_idx, map_indices)] = dn_cls_embed
    padding_bbox[(dn_b_idx, map_indices)] = dn_bbox

    tgt_size = num_dn + num_queries
    attn_mask = jt.zeros([tgt_size, tgt_size], dtype=jt.bool)
