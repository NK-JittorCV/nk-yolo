...
using SAM. It forms an integral part of the jittoryolo framework and is designed for high-performance, real-time image
segmentation tasks.
"""
...
class Predictor(BasePredictor):
    ...
    Attributes:
        ...
        im (jt.Var): The preprocessed input image.
        features (jt.Var): Extracted image features.
        ...
        mean (jt.Var): Mean values for image normalization.
        std (jt.Var): Standard deviation values for image normalization.
    ...
    def preprocess(self, im):
        ...
        jt.Var and list of np.ndarray as input formats.

        Args:
            im (jt.Var | List[np.ndarray]): Input image(s) in BCHW tensor format or list of HWC numpy arrays.

        Returns:
            im (jt.Var): The preprocessed image tensor, normalized and converted to the appropriate dtype.
        ...
        if self.im is not None:
            return self.im
        not_tensor = not isinstance(im, jt.Var)
        if not_tensor:
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        Args:
            im (jt.Var): The preprocessed input image in tensor format, with shape (N, C, H, W).
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        ...
        Args:
            im (jt.Var): Preprocessed input image tensor with shape (N, C, H, W).
    ...
    def _prepare_prompts(self, dst_shape, bboxes=None, points=None, labels=None, masks=None):
        ...
        if points is not None:
            points = jt.array(points, dtype='float32', device=self.device)
            points = points[None] if points.ndim == 1 else points
            # Assuming labels are all positive if users don't pass labels.
            if labels is None:
                labels = np.ones(points.shape[:-1])
            labels = jt.array(labels, dtype='int32', device=self.device)
            assert (
        ...
        if bboxes is not None:
            bboxes = jt.array(bboxes, dtype='float32', device=self.device)
            bboxes = bboxes[None] if bboxes.ndim == 1 else bboxes
            bboxes *= r
        if masks is not None:
            masks = jt.array(masks, dtype='float32', device=self.device).unsqueeze(1)
    ...
    def generate(
        ...
        Args:
            im (jt.Var): Input tensor representing the preprocessed image with shape (N, C, H, W).
        ...
        Returns:
            pred_masks (jt.Var): Segmented masks with shape (N, H, W).
            pred_scores (jt.Var): Confidence scores for each mask with shape (N,).
            pred_bboxes (jt.Var): Bounding boxes for each mask with shape (N, 4).
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            w, h = x2 - x1, y2 - y1
            area = jt.Var(w * h, device=im.device)
            points_scale = np.array([[w, h]])  # w, h
            # Crop image and interpolate to input size
            crop_im = jt.nn.interpolate(im[..., y1:y2, x1:x2], (ih, iw), mode="bilinear", align_corners=False)
            # (num_points, 2)
            ...
            for (points,) in batch_iterator(points_batch_size, points_for_image):
                ...
                pred_mask = jt.nn.interpolate(pred_mask[None], (h, w), mode="bilinear", align_corners=False)[0]
            ...
            crop_masks = jt.concat(crop_masks)
            crop_bboxes = jt.concat(crop_bboxes)
            crop_scores = jt.concat(crop_scores)
        ...
        pred_masks = jt.concat(pred_masks)
        pred_bboxes = jt.concat(pred_bboxes)
        pred_scores = jt.concat(pred_scores)
        region_areas = jt.concat(region_areas)
    ...
    def setup_model(self, model=None, verbose=True):
        ...
        parameters for image normalization and other jittoryolo compatibility settings.
        ...
        self.device = device
        self.mean = jt.Var([123.675, 116.28, 103.53]).view(-1, 1, 1).to(device)
        self.std = jt.Var([58.395, 57.12, 57.375]).view(-1, 1, 1).to(device)

        # jittoryolo compatibility settings
        self.model.pt = False
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            preds (Tuple[jt.Var]): The output from SAM model inference, containing:
                - pred_masks (jt.Var): Predicted masks with shape (N, 1, H, W).
                - pred_scores (jt.Var): Confidence scores for each mask with shape (N, 1).
                - pred_bboxes (jt.Var, optional): Predicted bounding boxes if segment_all is True.
            img (jt.Var): The processed input image tensor with shape (C, H, W).
            orig_imgs (List[np.ndarray] | jt.Var): The original, unprocessed images.
        ...
        for masks, orig_img, img_path in zip([pred_masks], orig_imgs, self.batch[0]):
            if len(masks) == 0:
                masks, pred_bboxes = None, jt.zeros((0, 6), dtype='float32', device=pred_masks.device)
                ...
                cls = jt.arange(len(pred_masks), dtype='int32', device=pred_masks.device)
                pred_bboxes = jt.concat([pred_bboxes, pred_scores[:, None], cls[:, None]], dim=-1)
    ...
    def remove_small_regions(masks, min_area=0, nms_thresh=0.7):
        ...
        Args:
            masks (jt.Var): Segmentation masks to be processed, with shape (N, H, W) where N is the number of
        ...
        Returns:
            new_masks (jt.Var): Processed masks with small regions removed, shape (N, H, W).
        ...
        new_masks = jt.concat(new_masks, dim=0)
        boxes = batched_mask_to_box(new_masks)
        keep = torchvision.ops.nms(boxes.float(), jt.array(scores), nms_thresh)
...
class SAM2Predictor(Predictor):
    ...
    Attributes:
        ...
        device (torch.device): The device (CPU or GPU) on which the model is loaded.
        features (Dict[str, jt.Var]): Cached image features for efficient inference.
    ...
    def _prepare_prompts(self, dst_shape, bboxes=None, points=None, labels=None, masks=None):
        ...
        if bboxes is not None:
            ...
            bbox_labels = jt.Var([[2, 3]], dtype='int32', device=bboxes.device).expand(len(bboxes), -1)
            # NOTE: merge "boxes" and "points" into a single "points" input
            # (where boxes are added at the beginning) to model.sam_prompt_encoder
            if points is not None:
                points = jt.concat([bboxes, points], dim=1)
                labels = jt.concat([bbox_labels, labels], dim=1)
            else:
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None):
        ...
        return pred_masks, jt.ones(len(pred_masks), dtype=pred_masks.dtype, device=pred_masks.device)
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            preds (Tuple[jt.Var]): The predictions from the model.
            img (jt.Var): The processed image tensor.
    ...
    def add_new_prompts(
        ...
        Args:
            ...
            points (jt.Var, Optional): The coordinates of the points of interest. Defaults to None.
            labels (jt.Var, Optional): The labels corresponding to the points. Defaults to None.
            masks (jt.Var, optional): Binary masks for the object. Defaults to None.
        ...
        return pred_masks.flatten(0, 1), jt.ones(1, dtype=pred_masks.dtype, device=pred_masks.device)
    ...
    def get_im_features(self, im, batch=1):
        ...
        Args:
            im (jt.Var): The input image tensor.
            batch (int, optional): The batch size for expanding features if there are multiple prompts. Defaults to 1.

        Returns:
            vis_feats (jt.Var): The visual features extracted from the image.
            vis_pos_embed (jt.Var): The positional embeddings for the visual features.
    ...
    def _run_single_frame_inference(
        ...
        Args:
            ...
            mask_inputs (jt.Var, Optional): Input binary masks. Defaults to None.
            ...
            prev_sam_mask_logits (jt.Var, Optional): Previous mask logits for the current object. Defaults to None.
        ...
        if maskmem_features is not None:
            current_out["maskmem_features"] = maskmem_features.to(
                dtype='float16', device=self.device, non_blocking=True
            )
    ...
    def _get_maskmem_pos_enc(self, out_maskmem_pos_enc):
        ...
        Args:
            out_maskmem_pos_enc (List[jt.Var] or None): The positional encoding for mask memory.
                Should be a list of tensors or None.

        Returns:
            out_maskmem_pos_enc (List[jt.Var]): The positional encoding for mask memory, either cached or expanded.
    ...
    def _get_empty_mask_ptr(self, frame_idx):
        ...
        Returns:
            (jt.Var): A tensor representing the dummy object pointer generated from the empty mask.
        """
        # Retrieve correct image features
        ...
        current_out = self.model.track_step(
            ...
            mask_inputs=jt.zeros((1, 1, *self.imgsz), dtype='float32', device=self.device),
            output_dict={},
            num_frames=self.inference_state["num_frames"],
    ...
    def _run_memory_encoder(self, batch_size, high_res_masks, object_score_logits, is_mask_from_pts):
        ...
        Args:
            batch_size (int): The batch size for processing the frame.
            high_res_masks (jt.Var): High-resolution masks for which to compute the memory.
            object_score_logits (jt.Var): Logits representing the object scores.
            is_mask_from_pts (bool): Indicates if the mask is derived from point interactions.

        Returns:
            (tuple[jt.Var, jt.Var]): A tuple containing the encoded mask features and positional encoding.
        """
        # Retrieve correct image features
        ...
        return maskmem_features.to(dtype='float16', device=self.device, non_blocking=True), maskmem_pos_enc
