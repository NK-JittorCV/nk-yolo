...
from jittoryolo.nn.modules import MLPBlock
...
class TwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        Args:
            image_embedding (jt.Var): Image to attend to, with shape (B, embedding_dim, H, W).
            image_pe (jt.Var): Positional encoding to add to the image, with same shape as image_embedding.
            point_embedding (jt.Var): Embedding to add to query points, with shape (B, N_points, embedding_dim).

        Returns:
            (Tuple[jt.Var, jt.Var]): Processed point_embedding and image_embedding.
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(self, queries: jt.Var, keys: jt.Var, query_pe: jt.Var, key_pe: jt.Var) -> Tuple[jt.Var, jt.Var]:
...
class Attention(nn.Module):
    ...
    Methods:
        _separate_heads: Separates input jt.Var into attention heads.
        _recombine_heads: Recombines separated attention heads.
        forward: Computes attention output for given query, key, and value jt.Vars.
    ...
    @staticmethod
    def _separate_heads(x: jt.Var, num_heads: int) -> jt.Var:
        """Separates the input jt.Var into the specified number of attention heads."""
        b, n, c = x.shape
        x = x.reshape(b, n, num_heads, c // num_heads)
        return x.transpose(1, 2)  # B x N_heads x N_tokens x C_per_head

    @staticmethod
    def _recombine_heads(x: jt.Var) -> jt.Var:
        """Recombines separated attention heads into a single jt.Var."""
        b, n_heads, n_tokens, c_per_head = x.shape
        x = x.transpose(1, 2)
        return x.reshape(b, n_tokens, n_heads * c_per_head)  # B x N_tokens x C

    def forward(self, q: jt.Var, k: jt.Var, v: jt.Var) -> jt.Var:
        """Applies multi-head attention to query, key, and value jt.Vars with optional downsampling."""
        # Input projections
        q = self.q_proj(q)
        k = self.k_proj(k)
        v = self.v_proj(v)
