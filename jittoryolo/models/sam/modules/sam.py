...
from jittoryolo.nn.modules import MLP
...
class SAM2Model(nn.Module):
    ...
    def __init__(
        ...
        if use_obj_ptrs_in_encoder:
            ...
            self.mask_downsample = jt.nn.Conv2d(1, 1, kernel_size=4, stride=4)
        ...
        self.maskmem_tpos_enc = jt.nn.Parameter(jt.zeros(num_maskmem, 1, 1, self.mem_dim))
        jt.nn.init.normal_(self.maskmem_tpos_enc, mean=0.0, std=0.02)

        # a single token to indicate no memory embedding from previous frames
        self.no_mem_embed = jt.nn.Parameter(jt.zeros(1, 1, self.hidden_dim))
        self.no_mem_pos_enc = jt.nn.Parameter(jt.zeros(1, 1, self.hidden_dim))
        jt.nn.init.normal_(self.no_mem_embed, mean=0.0, std=0.02)
        jt.nn.init.normal_(self.no_mem_pos_enc, mean=0.0, std=0.02)
        ...
        if self.pred_obj_scores and self.use_obj_ptrs_in_encoder:
            self.no_obj_ptr = jt.nn.Parameter(jt.zeros(1, self.hidden_dim))
            jt.nn.init.normal_(self.no_obj_ptr, mean=0.0, std=0.02)

        self.use_mlp_for_obj_ptr_proj = use_mlp_for_obj_ptr_proj
        self.no_obj_embed_spatial = None
        if no_obj_embed_spatial:
            self.no_obj_embed_spatial = jt.nn.Parameter(jt.zeros(1, self.mem_dim))
            jt.nn.init.normal_(self.no_obj_embed_spatial, mean=0.0, std=0.02)
        ...
        if compile_image_encoder:
            ...
            #TODO: check if this works with jittor
            self.image_encoder.forward = jt.compile(self.image_encoder.forward, optimize=3)
    ...
    def _build_sam_heads(self):
        ...
        if self.use_obj_ptrs_in_encoder:
            ...
            self.obj_ptr_proj = jt.nn.Linear(self.hidden_dim, self.hidden_dim)
            if self.use_mlp_for_obj_ptr_proj:
                self.obj_ptr_proj = MLP(self.hidden_dim, self.hidden_dim, self.hidden_dim, 3)
        else:
            self.obj_ptr_proj = jt.nn.Identity()

        if self.proj_tpos_enc_in_obj_ptrs:
            # a linear projection on temporal positional encoding in object pointers to
            # avoid potential interference with spatial positional encoding
            self.obj_ptr_tpos_proj = jt.nn.Linear(self.hidden_dim, self.mem_dim)
        else:
            self.obj_ptr_tpos_proj = jt.nn.Identity()
    ...
    def _forward_sam_heads(
        ...
        Args:
            backbone_features (jt.Var): Image features with shape (B, C, H, W).
            point_inputs (Dict[str, jt.Var] | None): Dictionary containing point prompts.
                'point_coords': Tensor of shape (B, P, 2) with float32 dtype, containing absolute
                    pixel-unit coordinates in (x, y) format for P input points.
                'point_labels': Tensor of shape (B, P) with int32 dtype, where 1 means positive clicks,
                    0 means negative clicks, and -1 means padding.
            mask_inputs (jt.Var | None): Mask of shape (B, 1, H*16, W*16), float or bool, with the
                same spatial size as the image.
            high_res_features (List[jt.Var] | None): List of two feature maps with shapes
        ...
        Examples:
            ...
            >>> point_inputs = {"point_coords": torch.rand(1, 2, 2), "point_labels": jt.Var([[1, 0]])}
        ...
        if point_inputs is not None:
            ...
            sam_point_coords = jt.zeros(B, 1, 2, device=device)
            sam_point_labels = -jt.ones(B, 1, dtype=jt.int32, device=device)
        ...
        if mask_inputs is not None:
            ...
            if mask_inputs.shape[-2:] != self.sam_prompt_encoder.mask_input_size:
                #TODO: check if this works with jittor
                sam_mask_prompt = jt.nn.interpolate(
                    mask_inputs.float(),
        ...
        if self.pred_obj_scores:
            ...
            low_res_multimasks = jt.where(is_obj_appearing[:, None, None], low_res_multimasks, NO_OBJ_SCORE)

        # convert masks from possibly bfloat16 (or float16) to float32
        # (older PyTorch versions before 2.1 don't support `interpolate` on bf16)
        low_res_multimasks = low_res_multimasks.float()
        high_res_multimasks = jt.nn.interpolate(
            low_res_multimasks,
        ...
        if multimask_output:
            ...
            batch_inds = jt.arange(B, device=device)
    ...
    def _use_mask_as_output(self, backbone_features, high_res_features, mask_inputs):
        ...
        low_res_masks = jt.nn.interpolate(
            high_res_masks,
        ...
        if not self.use_obj_ptrs_in_encoder:
            ...
            obj_ptr = jt.zeros(mask_inputs.size(0), self.hidden_dim, device=mask_inputs.device)
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                if pos_and_ptrs:
                    ...
                    if self.add_tpos_enc_to_obj_ptrs:
                        ...
                        obj_pos = jt.Var(pos_list, device=device)
    ...
    def _apply_non_overlapping_constraints(pred_masks):
        ...
        max_obj_inds = jt.argmax(pred_masks, dim=0, keepdim=True)
        # "batch_obj_inds": object index of each object slice (along dim 0) in `pred_masks`
        batch_obj_inds = jt.arange(batch_size, device=device)[:, None, None, None]
        keep = max_obj_inds == batch_obj_inds
        # suppress overlapping regions' scores below -10.0 so that the foreground regions
        # don't overlap (here sigmoid(-10.0)=4.5398e-05)
        pred_masks = jt.where(keep, pred_masks, jt.clamp(pred_masks, max=-10.0))
        return pred_masks
