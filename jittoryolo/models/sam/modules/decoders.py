...
from jittoryolo.nn.modules import MLP, LayerNorm2d
...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        Args:
            image_embeddings (jt.Var): Embeddings from the image encoder.
            image_pe (jt.Var): Positional encoding with the shape of image_embeddings.
            sparse_prompt_embeddings (jt.Var): Embeddings of the points and boxes.
            dense_prompt_embeddings (jt.Var): Embeddings of the mask inputs.
            multimask_output (bool): Whether to return multiple masks or a single mask.

        Returns:
            (Tuple[jt.Var, jt.Var]): A tuple containing:
                - masks (jt.Var): Batched predicted masks.
                - iou_pred (jt.Var): Batched predictions of mask quality.
...
class SAM2MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        Args:
            image_embeddings (jt.Var): Embeddings from the image encoder with shape (B, C, H, W).
            image_pe (jt.Var): Positional encoding with the shape of image_embeddings (B, C, H, W).
            sparse_prompt_embeddings (jt.Var): Embeddings of the points and boxes with shape (B, N, C).
            dense_prompt_embeddings (jt.Var): Embeddings of the mask inputs with shape (B, C, H, W).
            multimask_output (bool): Whether to return multiple masks or a single mask.
            repeat_image (bool): Flag to repeat the image embeddings.
            high_res_features (List[jt.Var] | None): Optional high-resolution features.

        Returns:
            (Tuple[jt.Var, jt.Var, jt.Var, jt.Var]): A tuple containing:
                - masks (jt.Var): Batched predicted masks with shape (B, N, H, W).
                - iou_pred (jt.Var): Batched predictions of mask quality with shape (B, N).
                - sam_tokens_out (jt.Var): Batched SAM token for mask output with shape (B, N, C).
                - object_score_logits (jt.Var): Batched object score logits with shape (B, 1).
    ...
    def _get_stability_scores(self, mask_logits):
        ...
        return jt.where(area_u > 0, area_i / area_u, 1.0)
    ...
    def _dynamic_multimask_via_stability(self, all_mask_logits, all_iou_scores):
        ...
        Args:
            all_mask_logits (jt.Var): Logits for all predicted masks, shape (B, N, H, W) where B is
                batch size, N is number of masks (typically 4), and H, W are mask dimensions.
            all_iou_scores (jt.Var): Predicted IoU scores for all masks, shape (B, N).

        Returns:
            (Tuple[jt.Var, jt.Var]):
                - mask_logits_out (jt.Var): Selected mask logits, shape (B, 1, H, W).
                - iou_scores_out (jt.Var): Selected IoU scores, shape (B, 1).
        ...
        batch_inds = jt.arange(multimask_iou_scores.size(0), device=all_iou_scores.device)
