...
from jittoryolo.nn.modules import LayerNorm2d
...
class ImageEncoderViT(nn.Module):
    ...
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.pos_embed = jt.nn.Parameter(jt.zeros(1, img_size // patch_size, img_size // patch_size, embed_dim))
    ...
    def forward(self, x: jt.Var) -> jt.Var:
        ...
        if self.pos_embed is not None:
            pos_embed = (
                jt.nn.interpolate(self.pos_embed.permute(0, 3, 1, 2), scale_factor=self.img_size / 1024).permute(0, 2, 3, 1)
...
class PromptEncoder(nn.Module):
    ...
    def get_dense_pe(self) -> jt.Var:
        ...
        Returns:
            (jt.Var): Positional encoding tensor with shape (1, embed_dim, H, W), where H and W are the
    ...
    def _embed_points(self, points: jt.Var, labels: jt.Var, pad: bool) -> jt.Var:
        """Embeds point prompts by applying positional encoding and label-specific embeddings."""
        points = points + 0.5  # Shift to center of pixel
        if pad:
            padding_point = jt.zeros((points.shape[0], 1, 2), device=points.device)
            padding_label = -jt.ones((labels.shape[0], 1), device=labels.device)
    ...
    def _get_batch_size(
        ...
        if points is not None:
            ...
            return 1
    #TODO:修改jt.Device
    def _get_device(self) -> jt.Device:
    ...
    def forward(
        ...
        Args:
            points (Tuple[jt.Var, jt.Var] | None): Point coordinates and labels to embed. The first
                tensor contains coordinates with shape (B, N, 2), and the second tensor contains labels with
                shape (B, N).
            boxes (jt.Var | None): Boxes to embed with shape (B, M, 2, 2), where M is the number of boxes.
            masks (jt.Var | None): Masks to embed with shape (B, 1, H, W).

        Returns:
            (Tuple[jt.Var, jt.Var]): A tuple containing:
                - sparse_embeddings (jt.Var): Sparse embeddings for points and boxes with shape (B, N, embed_dim).
                - dense_embeddings (jt.Var): Dense embeddings for masks of shape (B, embed_dim, embed_H, embed_W).
        ...
        sparse_embeddings = jt.empty((bs, 0, self.embed_dim), device=self._get_device())
        if points is not None:
...
class ImageEncoder(nn.Module):
    ...
    def forward(self, sample: jt.Var):
...
class FpnNeck(nn.Module):
    ...
    def forward(self, xs: List[jt.Var]):
        """
        ...
        Args:
            xs (List[jt.Var]): List of input tensors from the backbone, each with shape (B, C, H, W).

        Returns:
            (Tuple[List[jt.Var], List[jt.Var]]): A tuple containing:
                - out (List[jt.Var]): List of output feature maps after FPN processing, each with shape
                  (B, d_model, H, W).
                - pos (List[jt.Var]): List of positional encodings corresponding to each output feature map.
        ...
        for i in range(n, -1, -1):
            ...
            if i in self.fpn_top_down_levels and prev_features is not None:
                ...
                top_down_features = jt.nn.interpolate(
...
class Hiera(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = jt.nn.Parameter(jt.zeros(1, embed_dim, *self.window_pos_embed_bkg_spatial_size))
        self.pos_embed_window = jt.nn.Parameter(jt.zeros(1, embed_dim, self.window_spec[0], self.window_spec[0]))
    ...
    def _get_pos_embed(self, hw: Tuple[int, int]) -> jt.Var:
        ...
        #TODO: check if this is correct
        pos_embed = jt.nn.interpolate(self.pos_embed, size=(h, w), mode="bicubic")

        pos_embed = pos_embed + window_embed.tile([x // y for x, y in zip(pos_embed.shape, window_embed.shape)])
        pos_embed = pos_embed.permute(0, 2, 3, 1)
        return pos_embed

    def forward(self, x: jt.Var) -> List[jt.Var]:
