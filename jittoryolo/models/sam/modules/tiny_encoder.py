...
from jittoryolo.utils.instance import to_2tuple


class Conv2d_BN(jt.nn.Sequential):
    """
    ...
    def __init__(self, a, b, ks=1, stride=1, pad=0, dilation=1, groups=1, bn_weight_init=1):
        ...
        super().__init__()
        self.add_module("c", jt.nn.Conv2d(a, b, ks, stride, pad, dilation, groups, bias=False))
        bn = jt.nn.BatchNorm2d(b)
        jt.nn.init.constant_(bn.weight, bn_weight_init)
        jt.nn.init.constant_(bn.bias, 0)
...
class ConvLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            #TODO:check if jt.checkpoint is True
            x = jt.checkpoint(blk, x) if self.use_checkpoint else blk(x)
...
class Attention(jt.nn.Module):
    ...
    def __init__(
        ...
        self.attention_biases = jt.nn.Parameter(jt.zeros(num_heads, len(attention_offsets)))
        self.register_buffer("attention_bias_idxs", jt.int64(idxs).view(N, N), persistent=False)
...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if h == self.window_size and w == self.window_size:
            ...
            if padding:
                x = jt.nn.pad(x, [0, 0, 0, pad_r, 0, pad_b], mode='constant', value=0)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            #TODO : check if checkpointing is working
            x = jt.checkpoint(blk, x) if self.use_checkpoint else blk(x)
        return x if self.downsample is None else self.downsample(x)
...
class TinyViT(nn.Module):
    ...
    def __init__(
        ...
        self.head = nn.Linear(embed_dims[-1], num_classes) if num_classes > 0 else jt.nn.Identity()
