...
from jittoryolo.nn.modules import MLP, LayerNorm2d, MLPBlock
...
class CXBlock(nn.Module):
    ...
    def __init__(
        ...
        self.gamma = (
            nn.Parameter(layer_scale_init_value * jt.ones(dim))
...
class RoPEAttention(Attention):
    ...
    def forward(self, q: jt.Var, k: jt.Var, v: jt.Var, num_k_exclude_rope: int = 0) -> jt.Var:
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x: jt.Var) -> jt.Var:
        ...
        q, k, v = [jt.squeeze(t, dim=2) for t in jt.split(qkv, 1, dim=2)]
        ...
        attn_scores = (q.transpose(1, 2) @ k.transpose(1, 2).transpose(-2, -1)) / jt.sqrt(jt.float32(q.shape[-1]))
        attn = jt.softmax(attn_scores, dim=-1)
...
class PositionEmbeddingSine(nn.Module):
    ...
    def _encode_xy(self, x, y):
        ...
        dim_t = jt.arange(self.num_pos_feats, dtype=jt.float32).to(x.device)

        dim_t = self.temperature ** (2 * (dim_t // 2) / self.num_pos_feats)

        pos_x = x_embed[:, None] / dim_t
        pos_y = y_embed[:, None] / dim_t
        pos_x = jt.stack((jt.sin(pos_x[:, 0::2]), jt.cos(pos_x[:, 1::2])), dim=2).flatten(1)
        pos_y = jt.stack((jt.sin(pos_y[:, 0::2]), jt.cos(pos_y[:, 1::2])), dim=2).flatten(1)
    ...
    def encode_boxes(self, x, y, w, h):
        with jt.no_grad():
            ...
            return jt.concat((pos_y, pos_x, jt.unsqueeze(h, 1), jt.unsqueeze(w, 1)), dim=1)
    ...
    def encode_points(self, x, y, labels):
        with jt.no_grad():
            ...
            return jt.concat((pos_y, pos_x, jt.unsqueeze(labels, 2)), dim=2)
    ...
    def forward(self, x: jt.Var):
        with jt.no_grad():
            ...
            y_embed = (
                jt.arange(1, x.shape[-2] + 1, dtype=jt.float32).view(1, -1, 1).repeat(x.shape[0], 1, x.shape[-1])
            )
            x_embed = (
                jt.arange(1, x.shape[-1] + 1, dtype=jt.float32).view(1, 1, -1).repeat(x.shape[0], x.shape[-2], 1)
            )
...
#TODO:jt.arange(self.num_pos_feats, dtype=jt.float32).to(x.device) 中的 .to(x.device) 可能需要根据 Jittor 的设备管理方式进行调整
            dim_t = jt.arange(self.num_pos_feats, dtype=jt.float32).to(x.device)
            dim_t = self.temperature ** (2 * (dim_t // 2) / self.num_pos_feats)

            pos_x = x_embed[:, :, :, None] / dim_t
            pos_y = y_embed[:, :, :, None] / dim_t
            pos_x = jt.stack((jt.sin(pos_x[:, :, :, 0::2]), jt.cos(pos_x[:, :, :, 1::2])), dim=4).flatten(3)
            pos_y = jt.stack((jt.sin(pos_y[:, :, :, 0::2]), jt.cos(pos_y[:, :, :, 1::2])), dim=4).flatten(3)
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def __init__(self, num_pos_feats: int = 64, scale: Optional[float] = None) -> None:
        ...
        if scale is None or scale <= 0.0:
            scale = 1.0
        self.register_buffer("positional_encoding_gaussian_matrix", scale * jt.randn((2, num_pos_feats)))


        # Set non-deterministic for forward() error 'cumsum_cuda_kernel does not have a deterministic implementation'
        #TODO:确定 Jittor 的设备管理方式后，可以考虑使用 Jittor 的随机数生成器
    ...
    def forward(self, size: Tuple[int, int]) -> jt.Var:
    ...
    def forward_with_coords(self, coords_input: jt.Var, image_size: Tuple[int, int]) -> jt.Var:
...
class REAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_rel_pos:
            ...
            self.rel_pos_h = jt.nn.Parameter(jt.zeros(2 * input_size[0] - 1, head_dim))
            self.rel_pos_w = jt.nn.Parameter(jt.zeros(2 * input_size[1] - 1, head_dim))
