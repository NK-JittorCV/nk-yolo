...
def is_box_near_crop_edge(
    ...
    crop_box_jittor = jt.array(crop_box, dtype=jt.float32, device=boxes.device)
    orig_box_jittor = jt.array(orig_box, dtype=jt.float32, device=boxes.device)
    boxes = uncrop_boxes_xyxy(boxes, crop_box).float()
    near_crop_edge = jt.isclose(boxes, crop_box_jittor[None, :], atol=atol, rtol=0)
    near_image_edge = jt.isclose(boxes, orig_box_jittor[None, :], atol=atol, rtol=0)
    near_crop_edge = jt.logical_and(near_crop_edge, ~near_image_edge)
...
def calculate_stability_score(masks: jt.Var, mask_threshold: float, threshold_offset: float) -> jt.Var:
    """
    ...
    Args:
        masks (jt.Var): Batch of predicted mask logits.
    ...
    Returns:
        (jt.Var): Stability scores for each mask in the batch.
    ...
    """
    intersections = (masks > (mask_threshold + threshold_offset)).sum(-1, dtype=jt.int16).sum(-1, dtype=jt.int32)
    unions = (masks > (mask_threshold - threshold_offset)).sum(-1, dtype=jt.int16).sum(-1, dtype=jt.int32)
...
def uncrop_boxes_xyxy(boxes: jt.Var, crop_box: List[int]) -> jt.Var:
    ...
    x0, y0, _, _ = crop_box
    offset = jt.Var([[x0, y0, x0, y0]], device=boxes.device)
...
def uncrop_points(points: jt.Var, crop_box: List[int]) -> jt.Var:
    ...
    x0, y0, _, _ = crop_box
    offset = jt.Var([[x0, y0]], device=points.device)
...
def uncrop_masks(masks: jt.Var, crop_box: List[int], orig_h: int, orig_w: int) -> jt.Var:
    ...
    return jt.nn.pad(masks, pad, mode="constant", value=0)
...
def batched_mask_to_box(masks: jt.Var) -> jt.Var:
    ...
    if jt.numel(masks) == 0:
        #TODO: Check if this is the correct output shape
        return jt.zeros(masks.shape[:-2] + (4,), dtype='float32', device=masks.device)
    ...
    in_height_coords = in_height * jt.arange(h, device=in_height.device)[None, :]
    ...
    in_width_coords = in_width * jt.arange(w, device=in_width.device)[None, :]
