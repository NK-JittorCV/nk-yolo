...
Example:
    ```python
    from jittoryolo import NAS

    model = NAS("yolo_nas_s")
    results = model.predict("jittoryolo/assets/bus.jpg")
    ```
"""
...
class NAS(Model):
    ...
    This class provides an interface for the YOLO-NAS models and extends the `Model` class from jittoryolo engine.
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        if suffix == ".pt":
            self.model = jt.load(attempt_download_asset(weights))
        ...
        self.model.stride = jt.array([32], dtype=jt.int64)
