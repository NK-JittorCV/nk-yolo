...
from jittoryolo.data.utils import polygons2masks, polygons2masks_overlap
from jittoryolo.utils import LOGGER, colorstr
from jittoryolo.utils.checks import check_version
from jittoryolo.utils.instance import Instances
from jittoryolo.utils.metrics import bbox_ioa
from jittoryolo.utils.ops import segment2box, xyxyxyxy2xywhr
...
class BaseTransform:
    """
    Base class for image transformations in the jittoryolo library.
...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        Examples:
            >>> from jittoryolo.data.augment import Compose, RandomHSV, RandomFlip
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from jittoryolo.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from jittoryolo.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        values and labels. This implementation is designed for use with the jittoryolo YOLO framework.
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from jittoryolo.data.augment import RandomHSV
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing the following keys:
                ...
                'instances' (jittoryolo.utils.instance.Instances): An object containing bounding boxes and
                    optionally keypoints.

        Returns:
            (Dict): The same dictionary with the flipped image and updated instances:
                'img' (numpy.ndarray): The flipped image.
                'instances' (jittoryolo.utils.instance.Instances): Updated instances matching the flipped image.
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from jittoryolo.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
def classify_transforms(
    ...
    """
    import jittor.transform as T  # scope for faster 'import jittoryolo'
...
#     final_tfl = [
#         T.ToTensor(),
#         T.ImageNormalize(mean=jt.float_auto(mean), std=jt.float_auto(std)),
...
class ToTensor:
    ...
    def __init__(self, half=False):
        ...
        jittoryolo YOLO framework. It converts numpy arrays or PIL Images to PyTorch tensors, with an option
