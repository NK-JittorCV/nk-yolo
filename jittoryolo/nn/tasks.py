...
from jittoryolo.nn.modules import (
    AIFI,
    C1,
    C2,
    C2PSA,
...
from jittoryolo.utils.loss import (
    E2EDetectLoss,
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from jittoryolo.models.utils.loss import RTDETRDetectionLoss
...
class WorldModel(DetectionModel):
    ...
    def set_classes(self, text, batch=80, cache_clip_model=True):
        ...
        try:
            ...
            check_requirements("git+https://github.com/jittoryolo/CLIP.git")
            import clip
...
def jittor_safe_load(weight, safe_only=False):
    ...
    Example:
    ```python
    from jittoryolo.nn.tasks import jittor_safe_load
    ...
    try:
        with temporary_modules(
            modules={
                "jittoryolo.yolo.utils": "jittoryolo.utils",
                "jittoryolo.yolo.v8": "jittoryolo.models.yolo",
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"https://github.com/jittoryolo/yolov5.\nThis model is NOT compatible with "
                    f"YOLOv8 at https://github.com/jittoryolo/jittoryolo."
                    f"\nRecommend fixes are to train a new model using the latest 'jittoryolo' package or to "
                    f"run a command with an official YOLO model, i.e. 'yolo predict model=yolov8n.pt'"
                )
            ) from e
        LOGGER.warning(
            f"WARNING ⚠️ {weight} appears to require '{e.name}', which is not in Jittoryolo requirements."
