...
from jittoryolo.nn.tasks import attempt_load_one_weight, guess_model_task, nn, yaml_model_load
...
class Model(nn.Module):
    ...
    loaded from local files, jittoryolo HUB, or Triton Server.
    ...
    Attributes:
        ...
        session (HUBTrainingSession): The jittoryolo HUB session, if applicable.
    ...
    def __init__(
        ...
        model sources, including local files, jittoryolo HUB models, and Triton Server models. The method
        ...
        Args:
            model (Union[str, Path]): Path or name of the model to load or create. Can be a local file path, a
                model name from jittoryolo HUB, or a Triton Server model.
    ...
    def __call__(
        self,
        source: Union[str, Path, int, Image.Image, list, tuple, np.ndarray, jt.Var] = None,
        stream: bool = False,
        **kwargs,
    ) -> list:
        """
        ...
        Returns:
            (List[jittoryolo.engine.results.Results]): A list of prediction results, each encapsulated in a
                Results object.

        Examples:
            >>> model = YOLO("yolo11n.pt")
            >>> results = model("https://jittoryolo.com/images/bus.jpg")
            >>> for r in results:
    ...
    def save(self, filename: Union[str, Path] = "saved_model.pt") -> None:
        ...
        the date, jittoryolo version, license information, and a link to the documentation.
    ...
    def embed(
        self,
        source: Union[str, Path, int, list, tuple, np.ndarray, jt.Var] = None,
        stream: bool = False,
        **kwargs,
    ) -> list:
        """
        ...
        Examples:
            >>> model = YOLO("yolo11n.pt")
            >>> image = "https://jittoryolo.com/images/bus.jpg"
    ...
    def predict(
        ...
        **kwargs,
        ...
        is_cli = (ARGV[0].endswith("yolo") or ARGV[0].endswith("jittoryolo")) and any(
    ...
    def val(
        ...
        **kwargs,
        ...
        Args:
            validator (jittoryolo.engine.validator.BaseValidator | None): An instance of a custom validator class for
                validating the model.
            **kwargs (Any): Arbitrary keyword arguments for customizing the validation process.

        Returns:
            (jittoryolo.utils.metrics.DetMetrics): Validation metrics obtained from the validation process.
    ...
    def benchmark(
        ...
        **kwargs,
        ...
        It uses the 'benchmark' function from the jittoryolo.utils.benchmarks module. The benchmarking is
        ...
        """
        self._check_is_pytorch_model()
        from jittoryolo.utils.benchmarks import benchmark
    ...
    def train(
        ...
        **kwargs,
        ...
        from a checkpoint, integrating with jittoryolo HUB, and updating model and configuration after training.

        When using jittoryolo HUB, if the session has a loaded model, the method prioritizes HUB training
        ...
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and self.session.model.id:  # jittoryolo HUB session with loaded model
            if any(kwargs):
    ...
    def names(self) -> list:
        ...
        using the 'check_class_names' function from the jittoryolo.nn.autobackend module. If the predictor is not
        ...
        """
        from jittoryolo.nn.autobackend import check_class_names
    ...
    def clear_callback(self, event: str) -> None:
        ...
        Args:
            event (str): The name of the event for which to clear the callbacks. This should be a valid event name
                recognized by the jittoryolo callback system.
        ...
        Notes:
            - This method affects both custom callbacks added by the user and default callbacks
              provided by the jittoryolo framework.
    ...
    def task_map(self) -> dict:
        ...
        various tasks and modes within the jittoryolo framework.
        ...
        Note:
            ...
            classes supported by the jittoryolo framework. The docstring provides a general
