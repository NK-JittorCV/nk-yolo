...
# Refer to https://github.com/ultralytics/ultralytics/blob/main/ultralytics/cfg/__init__.py
...
CLI_HELP_MSG = f"""
    Arguments received: {str(['yolo'] + ARGV[1:])}. NK-YOLO 'yolo' commands use the following syntax:
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    ...
    Examples:
        >>> from nkyolo.cfg import get_cfg
...
def check_cfg(cfg, hard=True):
    """
    Checks configuration argument types and values for the NK-YOLO library.
...
def handle_yolo_hub(args: List[str]) -> None:
    """
    Handles NK-YOLO HUB command-line interface (CLI) commands for authentication.

    This function processes NK-YOLO HUB CLI commands such as login and logout. It should be called when executing a
    ...
    Notes:
        - The function imports the 'hub' module from NK-YOLO to perform login and logout operations.
        - For the 'login' command, if no API key is provided, an empty string is passed to the login function.
        - The 'logout' command does not require any additional arguments.
    """
    from nkyolo import hub

    if args[0] == "login":
        key = args[1] if len(args) > 1 else ""
        # Log in to NK-YOLO HUB using the provided API key
        hub.login(key)
    elif args[0] == "logout":
        # Log out from NK-YOLO HUB
        hub.logout()
...
def handle_streamlit_inference():
    ...
    NK-YOLO models. It checks for the required Streamlit package and launches the app.
    ...
    Notes:
        ...
        - The Streamlit app file is located in the NK-YOLO package directory.
    """
    checks.check_requirements("streamlit>=1.29.0")
    LOGGER.info("💡 Loading NK-YOLO Live Inference app...")
...
def entrypoint(debug=""):
    ...
    This function serves as the main entry point for the NK-YOLO CLI, parsing command-line arguments and
    ...
    if "rtdetr" in stem:  # guess architecture
        from nkyolo import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from nkyolo import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem:
        from nkyolo import SAM
