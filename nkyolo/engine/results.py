...
# Refer to https://github.com/ultralytics/ultralytics/blob/main/ultralytics/engine/results.py
"""
NK-YOLO Results, Boxes and Masks classes for handling inference results.
...
from nkyolo.utils import LOGGER, SimpleClass, ops
from nkyolo.utils.checks import check_requirements
from nkyolo.utils.plotting import Annotator, colors, save_one_box
...
class BaseTensor(SimpleClass):
    ...
    def __init__(self, data, orig_shape) -> None:
        ...
        Examples:
            ...
            >>> base_tensor = BaseTensor(data, orig_shape)
        """
        assert isinstance(data, (jt.Var, np.ndarray)), "data must be torch.Tensor or np.ndarray"
        self.data = data
    ...
    def cuda(self):
        ...
        Examples:
            >>> import torch
            >>> from nkyolo.engine.results import BaseTensor
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]])
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> gpu_tensor = base_tensor.cuda()
            ...
            cuda:0
        """
        return self.__class__(jt.array(self.data).cuda(), self.orig_shape)
    ...
    def to(self, *args, **kwargs):
        ...
        """
        return self.__class__(jt.array(self.data).to(*args, **kwargs), self.orig_shape)
...
class Results(SimpleClass):
    ...
    def plot(
        ...
        if img is None and isinstance(self.orig_img, jt.Var):
            img = (self.orig_img[0].detach().permute(1, 2, 0).contiguous() * 255).to(jt.uint8).cpu().numpy()
        ...
        if pred_masks and show_masks:
            if im_gpu is None:
                ...
                im_gpu = (
                    jt.array(img, dtype=jt.float16, device=pred_masks.data.device)
                    .permute(2, 0, 1)
                    .flip(0)
                    .contiguous()
                    / 255
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                if masks:
                    ...
                    line = (c, *seg)
                if kpts is not None:
                    kpt = jt.concat((kpts[j].xyn, kpts[j].conf[..., None]), 2) if kpts[j].has_visible else kpts[j].xyn
...
class Boxes(BaseTensor):
    ...
    def xyxyn(self):
        ...
        """
        xyxy = self.xyxy.clone() if isinstance(self.xyxy, jt.Var) else np.copy(self.xyxy)
...
class Masks(BaseTensor):
    ...
    def __init__(self, masks, orig_shape) -> None:
        ...
        Examples:
            >>> import torch
            >>> from nkyolo.engine.results import Masks
...
class Keypoints(BaseTensor):
    ...
    Examples:
        >>> import torch
        >>> from nkyolo.engine.results import Keypoints
    ...
    def xyn(self):
        ...
        Examples:
            ...
            torch.Size([1, 17, 2])
        """
        xy = self.xy.clone() if isinstance(self.xy, jt.Var) else np.copy(self.xy)
...
class OBB(BaseTensor):
    ...
    def xyxyxyxyn(self):
        ...
        Examples:
            ...
            torch.Size([10, 4, 2])
        """
        xyxyxyxyn = self.xyxyxyxy.clone() if isinstance(self.xyxyxyxy, jt.Var) else np.copy(self.xyxyxyxy)
    ...
    def xyxy(self):
        ...
        y = self.xyxyxyxy[..., 1]
        return (
            jt.stack([x.amin(1), y.amin(1), x.amax(1), y.amax(1)], -1)
