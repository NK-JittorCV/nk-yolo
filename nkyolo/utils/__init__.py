...
# Refer to https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/__init__.py
...
TQDM_BAR_FORMAT = (
    "{l_bar}{bar:10}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_noinv_fmt}{postfix}]"
    if VERBOSE
    else None
)  # tqdm bar format
LOGGING_NAME = "nkyolo"
...
HELP_MSG = """
    ...
    3. Use the command line interface (CLI):
        ...
        nkyolo 'yolo' CLI commands use the following syntax:
...
class TQDM(tqdm_original):
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for nkyolo projects.
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date NK-YOLO
            'default.yaml' file.\nPlease update your code with 'pip install -U nkyolo' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/ultralytics/ultralytics/blob/main/nkyolo/cfg/default.yaml
            """
        )
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the NK-YOLO library, setting the appropriate logging level and
    ...
    Examples:
        >>> set_logging(name="nkyolo", verbose=True)
        >>> logger = logging.getLogger("nkyolo")
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from nkyolo.utils import ThreadingLocked
...
USER_CONFIG_DIR = Path(os.getenv("YOLO_CONFIG_DIR") or get_user_config_dir())  # NK-YOLO settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    NK-YOLO TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting NK-YOLO settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        self.help_msg = (
            f"\nView NK-YOLO Settings with 'yolo settings' or at '{self.file}'"
            "\nUpdate Settings with 'yolo settings key=value', i.e. 'yolo settings runs_dir=path/to/dir'. "
            "For help see https://docs.ultralytics.com/quickstart/#nkyolo-settings."
        )

        super().__init__(self.file)

        if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
            LOGGER.info(f"Creating new NK-YOLO Settings v{version} file ✅ {self.help_msg}")
            self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "WARNING ⚠️ NK-YOLO settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent NK-YOLO package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"WARNING ⚠️ NK-YOLO setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No NK-YOLO setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(f"NK-YOLO setting '{k}' must be of type '{t}', not '{type(v)}'. {self.help_msg}")
        super().update(*args, **kwargs)
...
def vscode_msg(ext="nkyolo.nkyolo-snippets") -> str:
    """Display a message to install nkyolo-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.ultralytics.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view NK-YOLO VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("nkyolo: ")
...
from nkyolo.utils.patches import imread, imshow, imwrite, jittor_load, jittor_save
