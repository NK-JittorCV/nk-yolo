...
# Refer to https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/tal.py
...
class TaskAlignedAssigner(nn.Module):
    ...
    def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9):
        ...
        Args:
            xy_centers (jt.Tensor): Anchor center coordinates, shape (h*w, 2).
            gt_bboxes (jt.Tensor): Ground truth bounding boxes, shape (b, n_boxes, 4).
            eps (float, optional): Small value for numerical stability. Defaults to 1e-9.

        Returns:
            (jt.Tensor): Boolean mask of positive anchors, shape (b, n_boxes, h*w).
    ...
    def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
        ...
        Args:
            mask_pos (jt.Tensor): Positive mask, shape (b, n_max_boxes, h*w).
            overlaps (jt.Tensor): IoU overlaps, shape (b, n_max_boxes, h*w).
            n_max_boxes (int): Maximum number of ground truth boxes.

        Returns:
            target_gt_idx (jt.Tensor): Indices of assigned ground truths, shape (b, h*w).
            fg_mask (jt.Tensor): Foreground mask, shape (b, h*w).
            mask_pos (jt.Tensor): Updated positive mask, shape (b, n_max_boxes, h*w).
        ...
        if fg_mask.max() > 1:  # one anchor is assigned to multiple gt_bboxes
            ...
            max_overlaps_idx = jt.array(jt.argmax(overlaps, 1))[0,:,:]  # Changed from dim=1 to dim=1 (explicit)

            is_max_overlaps = jt.zeros(mask_pos.shape, dtype=mask_pos.dtype)
            is_max_overlaps.scatter_(1, max_overlaps_idx.unsqueeze(1), jt.Var(1))

            mask_pos = jt.where(mask_multi_gts, is_max_overlaps, mask_pos).float()  # (b, n_max_boxes, h*w)
            fg_mask = mask_pos.sum(1)  # Changed from dim=-2 to dim=1
            
        # Find each grid serve which gt(index)
        target_gt_idx = jt.array(jt.argmax(mask_pos, 1))[0,:,:]  # Changed from dim=-2 to dim=1
...
def dist2bbox(distance, anchor_points, xywh=True, dim=-1):
    ...
    if xywh:
        c_xy = (x1y1 + x2y2) / 2
        wh = x2y2 - x1y1
        return jt.cat((c_xy, wh), dim)  # xywh bbox
    return jt.cat((x1y1, x2y2), dim)  # xyxy bbox


def bbox2dist(anchor_points, bbox, reg_max):
    """Transform bbox(xyxy) to dist(ltrb)."""
    x1y1, x2y2 = bbox.chunk(2, -1)
    return jt.cat((anchor_points - x1y1, x2y2 - anchor_points), -1).clamp_(0, reg_max - 0.01)  # dist (lt, rb)
...
def dist2rbox(pred_dist, pred_angle, anchor_points, dim=-1):
    ...
    Args:
        pred_dist (jt.Tensor): Predicted rotated distance, shape (bs, h*w, 4).
        pred_angle (jt.Tensor): Predicted angle, shape (bs, h*w, 1).
        anchor_points (jt.Tensor): Anchor points, shape (h*w, 2).
        dim (int, optional): Dimension along which to split. Defaults to -1.

    Returns:
        (jt.Tensor): Predicted rotated bounding boxes, shape (bs, h*w, 4).
    """
    lt, rb = pred_dist.split(2, dim=dim)
    cos, sin = jt.cos(pred_angle), jt.sin(pred_angle)
    # (bs, h*w, 1)
    xf, yf = ((rb - lt) / 2).split(1, dim=dim)
    x, y = xf * cos - yf * sin, xf * sin + yf * cos
    xy = jt.cat([x, y], dim=dim) + anchor_points
    return jt.cat([xy, lt + rb], dim=dim)
