...
class BaseTransform:
    """
    Base class for image transformations in the NK-YOLO library.
...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        Examples:
            >>> from nkyolo.data.augment import Compose, RandomHSV, RandomFlip
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from nkyolo.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from nkyolo.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        values and labels. This implementation is designed for use with the NK-YOLO framework.
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from nkyolo.data.augment import RandomHSV
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing the following keys:
                ...
                'instances' (nkyolo.utils.instance.Instances): An object containing bounding boxes and
                    optionally keypoints.

        Returns:
            (Dict): The same dictionary with the flipped image and updated instances:
                'img' (numpy.ndarray): The flipped image.
                'instances' (nkyolo.utils.instance.Instances): Updated instances matching the flipped image.
...
class LetterBox:
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scale_fill=False, scaleup=True, center=True, stride=32, padding_value=114, interpolation=cv2.INTER_LINEAR):
        """
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from nkyolo.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
