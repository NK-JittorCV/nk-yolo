...
# Refer to https://github.com/ultralytics/ultralytics/blob/main/ultralytics/data/utils.py
...
from nkyolo.utils.checks import check_file, check_font, is_ascii
from nkyolo.utils.downloads import download, safe_download, unzip_file
from nkyolo.utils.ops import segments2boxes
...
class HUBDatasetStats:
    ...
    Example:
        ...
        ```python
        from nkyolo.data.utils import HUBDatasetStats
    ...
    def get_json(self, save=False, verbose=False):
        """Return dataset JSON for NK-YOLO HUB."""
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                from nkyolo.data import YOLODataset
    ...
    def process_images(self):
        """Compress images for NK-YOLO HUB."""
        from nkyolo.data import YOLODataset  # ClassificationDataset
...
def compress_one_image(f, f_new=None, max_dim=1920, quality=50):
    ...
    Example:
        ```python
        from pathlib import Path
        from nkyolo.data.utils import compress_one_image
...
def autosplit(path=DATASETS_DIR / "coco8/images", weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Example:
        ```python
        from nkyolo.data.utils import autosplit
...
def save_dataset_cache_file(prefix, path, x, version):
    ...
    if is_dir_writeable(path.parent):
        tmp_path = None
        # Write to a unique temp file, then atomically replace target.
        with tempfile.NamedTemporaryFile(prefix=path.stem, suffix=".npy", dir=path.parent, delete=False) as f:
            tmp_path = Path(f.name)
        np.save(str(tmp_path), x)
