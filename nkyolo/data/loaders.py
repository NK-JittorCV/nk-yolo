...
# Refer to https://github.com/ultralytics/ultralytics/blob/main/ultralytics/data/loaders.py
...
from nkyolo.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from nkyolo.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
from nkyolo.utils.checks import check_requirements
from nkyolo.utils.patches import imread
...
class LoadScreenshots:
    """
    jittoryolo screenshot dataloader for capturing and processing screen images.
...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, batch=1, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/jittoryolo/jittoryolo/issues/2912
            if "*" in a:
...
class LoadTensor:
    ...
    def _single_check(im, stride=32):
        ...
        if im.shape[2] % stride or im.shape[3] % stride: 
            raise ValueError(s)
        if im.max() > 1.0 + jt.finfo(im.dtype).eps:  # torch.float32 eps is 1.2e-07
            LOGGER.warning(
...
def autocast_list(source):
    """Merges a list of sources into a list of numpy arrays or PIL images for jittoryolo prediction."""
    files = []
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported jittoryolo prediction source type. \n"
                f"See https://docs.jittoryolo.com/modes/predict for supported source types."
            )
