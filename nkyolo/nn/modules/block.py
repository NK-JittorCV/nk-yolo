...
from nkyolo.utils.jittor_utils import fuse_conv_and_bn
...
class HGBlock(nn.Module):
    ...
    def execute(self, x):
        """execute pass of a PPHGNetV2 backbone layer."""
        y = [x]
        y.extend(m(y[-1]) for m in self.m)
        y = self.ec(self.sc(jt.concat(y, 1)))
...
class SPP(nn.Module):
    ...
    def execute(self, x):
        """execute pass of the SPP layer, performing spatial pyramid pooling."""
        x = self.cv1(x)
        return self.cv2(jt.concat([x] + [m(x) for m in self.m], 1))
...
class SPPF(nn.Module):
    ...
    def execute(self, x):
        """execute pass through Ghost Convolution block."""
        y = [self.cv1(x)]
        y.extend(self.m(y[-1]) for _ in range(3))
        return self.cv2(jt.concat(y, 1))
...
class C2(nn.Module):
    ...
    def execute(self, x):
        ...
        return self.cv2(jt.concat((self.m(a), b), 1))
...
class C2f(nn.Module):
    ...
    def execute_split(self, x):
        """execute pass using split() instead of chunk()."""
...
class BottleneckCSP(nn.Module):
    ...
    def execute(self, x):
        ...
        y2 = self.cv2(x)
        return self.cv4(self.act(self.bn(jt.concat((y1, y2), 1))))
...
class ResNetBlock(nn.Module):
    ...
    def execute(self, x):
        ...
        return jt.nn.relu(self.cv3(self.cv2(self.cv1(x))) + self.shortcut(x))
...
class ResNetLayer(nn.Module):
    ...
    def execute(self, x):
        """execute pass through the ResNet layer."""
        return self.layer(x)
...
class MaxSigmoidAttnBlock(nn.Module):
    ...
    def execute(self, x, guide):
        ...
        aw = jt.matmul(embed, guide.transpose(1, 2)).transpose(2, 3)
        aw = aw.max(dim=-1)[0]
        aw = aw / (self.hc**0.5)
...
class C2fAttn(nn.Module):
    ...
    def execute_split(self, x, guide):
        """execute pass using split() instead of chunk()."""
...
class ImagePoolingAttn(nn.Module):
    ...
    def __init__(self, ec=256, ch=(), ct=512, nh=8, k=3, scale=False):
        ...
        self.scale = nn.Parameter(jt.Var([0.0]), requires_grad=True) if scale else 1.0
    ...
    def execute(self, x, text):
        ...
        x = jt.concat(x, dim=-1).transpose(1, 2)
        q = self.query(text)
        k = self.key(x)
        v = self.value(x)
        ...
        aw = jt.matmul(q, k.transpose(1, 2))
        aw = aw / (self.hc**0.5)
        aw = jt.nn.softmax(aw, dim=-1)

        x = jt.matmul(aw, v.transpose(2, 3))
...
class ContrastiveHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.logit_scale = jt.nn.Parameter(jt.ones([]) * jt.log(jt.Var(1 / 0.07)))


    def execute(self, x, w):
        """execute function of contrastive learning."""
        x = jt.misc.normalize(x, p=2, dim=1, eps=1e-30)
        w = jt.misc.normalize(w, p=2, dim=-1, eps=1e-30)


        x = jt.matmul(x, w.transpose(1, 2)).transpose(2, 3)
...
class BNContrastiveHead(nn.Module):
    ...
    def __init__(self, embed_dims: int):
        ...
        self.logit_scale = jt.nn.Parameter(-1.0 * jt.ones([]))


    def execute(self, x, w):
        """execute function of contrastive learning."""
        x = self.norm(x)
        w = jt.misc.normalize(w, dim=-1, p=2)
        x = jt.matmul(x, w.transpose(1, 2)).transpose(2, 3)
...
class AConv(nn.Module):
    ...
    def execute(self, x):
        ...
        x = jt.nn.avg_pool2d(x, 2, 1, 0, False, True)
        return self.cv1(x)
...
class ADown(nn.Module):
    ...
    def execute(self, x):
        ...
        x1, x2 = x.chunk(2, 1)
        x1 = self.cv1(x1)
        x2 = jt.nn.max_pool2d(x2, 3, 2, 1)
        x2 = self.cv2(x2)
        return jt.concat((x1, x2), 1)
...
class CBLinear(nn.Module):
    ...
    def execute(self, x):
        """execute pass through CBLinear layer."""
...
class CBFuse(nn.Module):
    ...
    def execute(self, xs):
        """execute pass through CBFuse layer."""
        target_size = xs[-1].shape[2:]
        res = [jt.nn.interpolate(x[self.idx[i]], size=target_size, mode="nearest") for i, x in enumerate(xs[:-1])]
        return jt.sum(jt.stack(res + xs[-1:]), dim=0)
...
class RepVGGDW(jt.nn.Module):
    ...
    def execute(self, x):
        """
        Performs a execute pass of the RepVGGDW block.

        Args:
            x (jt.Var): Input tensor.

        Returns:
            (jt.Var): Output tensor after applying the depth wise separable convolution.
        """
        return self.act(self.conv(x) + self.conv1(x))

    def execute_fuse(self, x):
        """
        Performs a execute pass of the RepVGGDW block without fusing the convolutions.
    ...
    def fuse(self):
        ...
        conv1_w = jt.nn.pad(conv1_w, [2, 2, 2, 2])
...
class CIB(nn.Module):
    ...
    def execute(self, x):
        """
        execute pass of the CIB module.

        Args:
            x (jt.Var): Input tensor.

        Returns:
            (jt.Var): Output tensor.
        """
...
class Attention(nn.Module):
    ...
    def execute(self, x):
        """
        execute pass of the Attention module.

        Args:
            x (jt.Var): The input tensor.

        Returns:
            (jt.Var): The output tensor after self-attention.
        """
        B, C, H, W = x.shape
        N = H * W
        qkv = self.qkv(x)
...
class PSABlock(nn.Module):
    ...
    This class encapsulates the functionality for applying multi-head attention and feed-execute neural network layers
    with optional shortcut connections.

    Attributes:
        attn (Attention): Multi-head attention module.
        ffn (nn.Sequential): Feed-execute neural network module.
        add (bool): Flag indicating whether to add shortcut connections.

    Methods:
        execute: Performs a execute pass through the PSABlock, applying attention and feed-execute layers.

    Examples:
        Create a PSABlock and perform a execute pass
    ...
    def execute(self, x):
        """Executes a execute pass through PSABlock, applying attention and feed-execute layers to the input tensor."""
...
class PSA(nn.Module):
    ...
    This class encapsulates the functionality for applying position-sensitive attention and feed-execute networks to
    ...
    Attributes:
        ...
        ffn (nn.Sequential): Feed-execute network for further processing.

    Methods:
        execute: Applies position-sensitive attention and feed-execute network to the input tensor.
    ...
    def execute(self, x):
        ...
        b = b + self.attn(b)
        b = b + self.ffn(b)
        return self.cv2(jt.concat((a, b), 1))
...
class C2PSA(nn.Module):
    ...
    capabilities. It includes a series of PSABlock modules for self-attention and feed-execute operations.

    Attributes:
        c (int): Number of hidden channels.
        cv1 (Conv): 1x1 convolution layer to reduce the number of input channels to 2*c.
        cv2 (Conv): 1x1 convolution layer to reduce the number of output channels to c.
        m (nn.Sequential): Sequential container of PSABlock modules for attention and feed-execute operations.

    Methods:
        execute: Performs a execute pass through the C2PSA module, applying attention and feed-execute operations.
...
class C2fPSA(C2f):
    ...
    Methods:
        execute: Performs a execute pass through the C2fPSA module.
        execute_split: Performs a execute pass using split() instead of chunk().
...
class SCDown(nn.Module):
    ...
    Methods:
        execute: Applies the SCDown module to the input tensor.
...
class TorchVision(nn.Module):
    ...
    def __init__(self, c1, c2, model, weights="DEFAULT", unwrap=True, truncate=2, split=False):
        ...
        if unwrap:
            ...
            self.m = jt.nn.Sequential(*layers)
            self.split = split
        else:
            self.split = False
            self.m.head = self.m.heads = jt.nn.Identity()


    def execute(self, x):
        """execute pass through the model."""
        if self.split:
            y = [x]
            y.extend(m(y[-1]) for m in self.m)
        else:
...
class AAttn(nn.Module):
    ...
    def execute(self, x: jt.Var) -> jt.Var:
        ...
        qkv = self.qkv(x).reshape(B, -1, H * W).transpose(0, 2, 1)
        if self.area > 1:
