...
class Conv2(Conv):
    ...
    def fuse_convs(self):
        ...
        w = jt.zeros_like(self.conv.weight.data)
        i = [x // 2 for x in w.shape[2:]]
        w[:, :, i[0] : i[0] + 1, i[1] : i[1] + 1] = self.cv2.weight.data.clone()
        self.conv.weight.data += w
        self.__delattr__("cv2")
        self.execute = self.execute_fuse
...
class GhostConv(nn.Module):
    ...
    def execute(self, x):
        ...
        y = self.cv1(x)
        return jt.concat((y, self.cv2(y)), 1)
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if isinstance(branch, Conv):
            ...
            if not hasattr(self, "id_tensor"):
                ...
                self.id_tensor = jt.array(kernel_value).to(branch.weight.device)
...
class ChannelAttention(nn.Module):
    ...
    def execute(self, x: jt.Var) -> jt.Var:
        """Applies execute pass using activation on convolutions of the input, optionally using batch normalization."""
...
class SpatialAttention(nn.Module):
    ...
    def execute(self, x):
        ...
        return x * self.act(self.cv1(jt.concat([jt.mean(x, 1, keepdim=True), jt.max(x, 1, keepdim=True)[0]], 1)))
...
class CBAM(nn.Module):
    ...
    def execute(self, x):
        """Applies the execute pass through C1 module."""
