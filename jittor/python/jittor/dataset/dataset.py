...
class Dataset(object):
    ...
    def _worker_main(self, worker_id, buffer, status):
        import jittor_utils
        # Data-loader worker is a plain data process, never a distributed compute rank.
        # Disable MPI/CUDA state inherited from mpirun parent to avoid MPI+subprocess hangs.
        for k in (
        ...
        if jt.in_mpi:
            try:
                ...
                jt.in_mpi = False
                if hasattr(jt, "mpi") and hasattr(jt.mpi, "set_state"):
                    jt.mpi.set_state(False)
            except Exception:
                pass
        seed = jt.get_seed()
